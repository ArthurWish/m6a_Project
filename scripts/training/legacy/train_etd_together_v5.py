...
from scripts.training.configs.train_etd_together_bias_v5 import CONFIG as BIAS_CONFIG
from scripts.training.configs.train_etd_together_v4_baseline import CONFIG as BASELINE_CONFIG
...
def main():
    ...
    sb_cfg = StructBiasConfig(
        enabled=config.get("use_struct_bias", False),
        mode=config.get("struct_bias_backend", "online"),
        scale=config.get("struct_bias_scale", 1.0),
        n_downsample=n_downsample,
        window_size=2 * config["half_window"] + 1,
        rnafold_bin=config.get("online_rnafold_bin", "RNAfold"),
        rnafold_timeout=config.get("online_rnafold_timeout_seconds", 240),
    ...
    num_threads_per_worker = int(config.get("online_rnafold_batch_workers", 4))
    num_workers = int(config.get("dataloader_workers", 8))
    
    collate_obj = ETDCollate(
        use_struct_bias=config.get("use_struct_bias", False),
        struct_mode=str(config.get("struct_bias_backend", "online")).strip().lower(),
        struct_scale=float(config.get("struct_bias_scale", 1.0)),
        n_downsample=n_downsample,
        window_size=2 * config["half_window"] + 1,
        rnafold_kwargs={
            "rnafold_bin": config.get("online_rnafold_bin", "RNAfold"),
            "timeout_seconds": int(config.get("online_rnafold_timeout_seconds", 240)),
            "cache_size": int(config.get("online_rnafold_cache_size", 4096)),
        },
        struct_cache_dir=config.get("offline_struct_cache_dir", ""),
    ...
    val_loader = DataLoader(
        val_ds, batch_size=config["val_batch_size"], sampler=val_sampler,
        shuffle=False, collate_fn=collate_obj, num_workers=num_workers, pin_memory=True,
    )
    ...
    for epoch in range(1, config["epochs"] + 1):
        ...
        for step, batch in enumerate(train_loader, 1):
            ...
            out = model(
                tokens=tokens, attn_mask=attn_mask, center_indices=center_indices, attn_bias=attn_bias,
                m6a_det_positions=batch["m6a_det_positions"].to(device), m6a_det_mask=batch["m6a_det_mask"].to(device),
                extra_pos_positions=batch["extra_pos_positions"].to(device), extra_pos_mask=batch["extra_pos_mask"].to(device),
                m6a_neg_positions=batch["m6a_neg_positions"].to(device), m6a_neg_mask=batch["m6a_neg_mask"].to(device),
                clean_neg_positions=batch["clean_neg_positions"].to(device), clean_neg_mask=batch["clean_neg_mask"].to(device),
            )
